...
| PAGE-PREVIEW | 预览页 (preview) | src/qml_gui/pages/PreviewPage.qml + LayerSlider/MoveSlider/Legend/StatsPanel | GUI_Preview.cpp + GCodeViewer.cpp + IMSlider.cpp | Hybrid | manual-visual | 1 | v3.6 §3 已逐区域登记；PREV-LEFT 盘缩略图已收口（2026-09-15 PreviewPage leftPanel 顶部盘摘要块 + stateChanged 重读 + 盘颜色回退 + PREV-LEFT 审计断言）；R-P1.B 三件已核实修复 |
