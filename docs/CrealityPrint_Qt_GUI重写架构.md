# CrealityPrint 7.0 — Qt/QML GUI 重写架构说明

> 注：本文档沿用 CrealityPrint 时代命名，路径已更新到 third_party/OrcaSlicer。
...
| 3.1  | 2026-03-10 | **口径校正**：明确上游 CrealityPrint 源码为功能真值；新增迁移适配层口径；校正“骨架完成”与“功能完成”区别                      |
...
截至 2026-03-10，项目已经完成大量基础承接工作，但尚未达到“CrealityPrint 7.x 全功能 Qt6 还原”状态。后续所有架构、任务和进度说明必须遵守以下口径：

- `third_party/OrcaSlicer` 是功能真值与行为基线。
- 当前 `src/qml_gui` + `src/core` 是承接层，不是独立定义产品行为的新真值。
- “文件存在 / 页面可打开 / 主链路可跑通”只代表基础承接完成，不代表上游功能完成迁移。
- 进度必须以“上游对照验收”记账，而不是以“阶段骨架已落地”记账。
...
third_party/OrcaSlicer/src/slic3r/GUI
    ↓
Qt Migration Adapters / Services / ViewModels
src/core/**
    ↓
Qt6/QML Pages / Panels / Components
src/qml_gui/**
    ↓
Runtime Validation / Tests / Diagnostics
tests/** + scripts/auto_verify_with_vcvars.ps1
```

该关系是本项目后续所有模块设计与任务拆解的前提。
...
旧版文档默认采用“Qt6 工程自身的阶段性设计”来描述系统。经过本轮分析，现统一调整为“源码对照迁移架构”：

- 上游 GUI 模块定义业务行为、页面结构、配置语义和交互真值。
- Qt6 工程负责提供新的视图技术栈、状态桥接、线程桥接与视觉升级。
- 任何尚未经过上游对照验收的模块，都只能称为“已建立基础承接”，不能称为“已完成迁移”。
...
│ third_party/OrcaSlicer/src/slic3r/GUI / libslic3r   │
└─────────────────────────────────────────────────────────┘
                         │
┌─────────────────────────────────────────────────────────┐
│            Migration Adapter / Domain Service Layer    │
│  Qt Facade / Service / Thread Bridge / Data Mapping    │
└─────────────────────────────────────────────────────────┘
                         │
┌─────────────────────────────────────────────────────────┐
│                    ViewModel Layer                     │
│    EditorVM / PreviewVM / ConfigVM / MonitorVM ...     │
└─────────────────────────────────────────────────────────┘
                         │
┌─────────────────────────────────────────────────────────┐
│               QML Presentation / Interaction           │
│   main.qml / Pages / Panels / Components / Controls    │
└─────────────────────────────────────────────────────────┘
                         │
┌─────────────────────────────────────────────────────────┐
│              OpenGL / Runtime Validation Layer         │
│ GLViewport / GCodeRenderer / Tests / Diagnostics       │
└─────────────────────────────────────────────────────────┘
````
...
| 状态         | 定义                                        |
| ------------ | ------------------------------------------- |
| 基础承接完成 | 已有 Qt6 页面、服务或渲染链路，能运行主流程 |
| 迁移进行中   | 已部分对照上游，仍缺行为、作用域或复杂交互  |
| 迁移完成     | 已完成上游模块级对照与回归验收              |

目前多数核心模块位于“基础承接完成”或“迁移进行中”，尚未进入“迁移完成”。
├─ // 注册 OpenGL 自定义项
