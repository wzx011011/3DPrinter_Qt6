...
遵循 `.codex/rules/source-truth-migration.md` 中的规范，不在此重复全局规则。
...
- **每完成 2 个任务**，必须将恢复点写入 MEMORY.md（路径: `C:\Users\67376\.Codex\projects\E--ai-3DPrinter-Qt6\memory\MEMORY.md`）。MEMORY.md 是唯一 compact 后保证可用的持久化机制
- 恢复点格式（写入 MEMORY.md）：
