# Requirements Archive: v3.8 RHI Gizmo Parity
...
- [x] **GMATH-01:** Pure-math gizmo functions (computeRay, rayToAxisT, computeRotateAngle, rayXZIntersect) are extracted into a dependency-free library with unit tests covering each function.
- [x] **GMATH-02:** Axis-pick functions (pickMoveAxis, pickRotateAxis, pickScaleAxis) are extracted, parameterized (no member-variable coupling), and unit-tested.
- [x] **GMATH-03:** GLViewportRenderer continues to work identically after refactoring to call the extracted GizmoMath library (equivalence verified by behavior).

### Gizmo Geometry

- [x] **GGEO-01:** Move gizmo geometry (3 axis arrows: shaft lines + cone triangles) is generated as pure CPU vertex data, decoupled from any GL/RHI calls.
- [x] **GGEO-02:** Rotate gizmo geometry (3 torus rings) and scale gizmo geometry (3 axis shafts + box handles) are generated as pure CPU vertex data.
- [x] **GGEO-03:** Geometry generators produce vertex counts identical to the GL originals; snapshot tests pin the layout.

### RHI State Wiring

- [x] **GWIRE-01:** RhiViewportRenderer::synchronize reads gizmoMode, cutAxis, cutPosition, and computes gizmoCenter from the selected object's AABB. The broken state pipeline is connected.
- [x] **GWIRE-02:** RhiViewport::setGizmoMode (and cutAxis/cutPosition setters) call update() so the renderer re-syncs on state change.

### Move Gizmo

- [x] **GMOV-01:** Setting gizmoMode to Move renders three colored axis arrows (X red, Y green, Z blue) at the selected object's position on the default D3D11 path.
- [x] **GMOV-02:** The gizmo renders on top of geometry (not occluded by the model) using depth-clear or no-depth-write.
- [x] **GMOV-03:** Clicking a gizmo axis selects that axis for dragging (pickMoveAxis via GizmoMath).
- [x] **GMOV-04:** Dragging the selected axis moves the object along that axis only; camera orbit is not triggered during gizmo drag.

### Rotate + Scale Gizmos

- [x] **GROT-01:** Rotate gizmo renders 3 torus rings; dragging a ring rotates the object around that axis.
- [x] **GROT-02:** Rotate pick precision matches the GL path (ray-plane + ring radius test).
- [x] **GSCA-01:** Scale gizmo renders 3 axis shafts with box handles; dragging a handle scales the object along that axis.
- [x] **GSCA-02:** Scale pick precision matches the GL path.
...
|---|---|---|
| GMATH-01 | Phase 65 | Complete |
| GMATH-02 | Phase 65 | Complete |
| GMATH-03 | Phase 65 | Complete |
| GGEO-01 | Phase 66 | Complete |
| GGEO-02 | Phase 66 | Complete |
| GGEO-03 | Phase 66 | Complete |
| GWIRE-01 | Phase 67 | Complete |
| GWIRE-02 | Phase 67 | Complete |
| GMOV-01 | Phase 68 | Complete |
| GMOV-02 | Phase 68 | Complete |
