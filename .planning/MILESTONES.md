...
**Audit status:** `tech_debt` — 31/32 requirements satisfied, 13/13 phases verified, 6/6 integration chains wired (2 documented partials), no blockers.

**Key accomplishments:**

- **OpenVDB officially unlocked (Phase 142, VDB-01/02):** the v4.x "OpenVDB unavailable" premise — which blocked Hollow, SlaSupports, FaceDetector, and downstream OpenVDB consumers for 4 milestone cycles — was wrong. OpenVDB 8.2.0 was built and present in DEPS_PREFIX all along; the v4.x fork dropped `find_package(OpenVDB)` + renamed the gating target to `openvdb_libs` (which no file ever created). Fixed with 3 CMake changes: explicit `OPENVDB_LIBRARYDIR`, find_package AFTER libslic3r (avoids TBB export collision), libnoise NOTFOUND-sentinel force-fix.
- **Tech-debt closure (Phase 141, DEBT-01..05):** CGAL-02 true intersection (swap minus→cgal::intersect + don't delete tool for intersection); orphaned "网格布尔运算" CxMenuItem + meshBooleanSelected stub removed; drillObject C4715 fix; ASM rotate/scale live-visual compose (full transform matrix in RhiViewportRenderer); v50TechDebtRegressionLocked slot.
- **Hollow gizmo UI scaffolding (Phase 143, VDB-03/04/05):** case 8 returns hasSingleObject; "Blocked: OpenVDB unavailable" tooltip removed; Hollow button in GLToolbars; Hollow panel binds 6 Q_PROPERTYs. VDB-06 (SLA slice) deferred to v5.1+ sub-milestone (requires SLAPrint wiring from scratch).
- **Emboss complete (Phases 144-146, EMB-01..07):** real text2shapes+polygons2model pipeline was already wired pre-v5.0 (stb_truetype, no freetype dep); Phase 144 parameterized it (font enumeration via Emboss::get_font_list + height/depth from Q_PROPERTYs). Phase 145 added async Qt Concurrent wrapper (cancellation, off-thread text2shapes, GUI-thread result). Phase 146 added no-selection fallback + verified SVG path + 3MF geometry round-trip.
- **Preset bundle full chain (Phases 147-149, PSET-01..07):** exportBundleIni/importBundleIni (upstream-compatible per-preset `.ini`); CreatePresetsDialog (scope selector + inherits-from + name); verified Simple/Advanced C++ filter + UnsavedChangesDialog; comparePresets primitive; dirty-state propagation locked.
- **PartPlate UI completion (Phases 150-152, PLATE-01..06):** read-only gap analysis revealed PLATE-03/04 already done; Phase 151 added drag-to-reorder + verify-locks; Phase 152 locked all 6 plate-state staging buffers + PartPlate::m_config via source-audit.
- **Cross-workstream regression gate (Phase 153, REGRESS-04):** consolidated v50RegressionLocked slot + 11 per-phase slots; 280/280 tests passing across 4 core groups; re-asserts v4.6/v4.7/v4.8 anchors.

**Known deferred items at close (see audit + per-phase SUMMARYs):**
- VDB-06 (SLA slice path): v5.1+ SLA sub-milestone (SliceService is FFF-only, no SLA presets bundled — SLAPrint needs wiring from scratch).
- EMB-03 (async): minimal Qt Concurrent wrapper, not full upstream EmbossJob port (1586 lines + Job base system).
- EMB-06 (3MF text metadata): geometry round-trips; editable-text metadata deferred.
- PSET-05 (diff view): comparePresets primitive shipped; QML side-by-side consumer deferred.
- PLATE-05 (runtime thumbnails): persisted-plate thumbnails work; session-created-plate capture deferred.
- PLATE-06 (live ctest): source-audit locked; live multi-plate round-trip ctest deferred (test infra lacks PresetServiceMock fixture).
...
## v4.8 Dependency Unlock, Assembly Transform & i18n Completion (Shipped: 2026-07-16)

**Phases completed:** 5 phases (136-140), 8 plans, 7 requirements
**Git range:** 19 commits, 49 files, +9245/-4364
**Audit status:** `tech_debt` �� 7/7 requirements satisfied, no blockers.

**Key accomplishments:**

- **CGAL MeshBoolean + Drill activated on CGAL 5.4** (Phases 136/137, CGAL-01/02/03): the v4.7 "CGAL 5.6+ required" premise was wrong �� MeshBoolean uses `corefinement.h` which CGAL 5.4 already ships. A 2-line compat patch in the `third_party/OrcaSlicer` submodule (`#if 0`��`#if 1` + output iterators) + CMake re-enable sufficed; no dependency-bundle upgrade. `kCgalMeshBooleanAvailable` flipped true; `booleanObject`/`drillObject` call real `MeshBoolean::minus`/`self_union`; the return-false `MeshBoolean_mcut_stub.cpp` removed (LNK2005). Union + difference + drill work end-to-end; **intersection is a documented tech-debt gap** (silently subtracts �� `ProjectServiceMock.cpp:3153-3156`).
- **Assembly-mode per-volume move/rotate/scale** (Phase 138, ASM-01): a routing/wiring gap, not new gizmo machinery �� the existing Move/Rotate/Scale gizmo infrastructure is canvas-agnostic. 4 plans: (01) `ProjectServiceMock` assemble-transform accessors proxying to `ModelInstance::m_assemble_transformation` (Model.hpp:1280-1294) + `AssembleTransformCommand` (undo id 7); (02) `EditorViewModel` `availableGizmoMask` widening on AssembleView + 9 gizmo slot branches routing to the assemble transform when `m_activeCanvasType==2`; (03) `RhiViewportRenderer` per-object assemble-offset thread-through (translate-only) + `AssemblePage` Move/Rotate/Scale selector + gizmo drag-signal wiring; (04) real 3MF `<assemble>` block round-trip test (`testAssembleTransformRoundTrip`). Both success criteria met (automated). Rotate/scale live-visual compose deferred (translate-only render; transforms persist + round-trip).
- **en.ts full English translation** (Phase 139, I18N-04): filled all 1372 unfinished translations (1218 unique sources: 975 CJK + 243 non-CJK) via a re-runnable Python script (`scripts/translate_en_ts.py`). 0 unfinished; `en.qm`=148KB. **de/fr/ja/ko advanced 44%** (I18N-05, 723 core terms each via `scripts/translate_core_i18n.py`); ~906 remaining per language documented.
- **v4.8 regression gate** (Phase 140, REGRESS-03): `v48CrossWorkstreamRegressionLocked` source-audit slot locks all v4.8 anchors AND re-asserts v4.7/v4.6 anchors (paint gate, flatten, fixMesh, calibration modes) �� no regression. Canonical build exit 0, 5/5 ctest PASS, E2E PASS.

**Known deferred items at close:** CGAL-02 intersection boolean (returns subtraction) + orphaned `meshBooleanSelected` menu stub (2 CGAL-02 warnings from the integration-checker); assemble rotate/scale live-visual compose; de/fr/ja/ko long tail (~906/lang); `drillObject` C4715; 2-line CGAL submodule compat patch.

**Archives:**

- `.planning/milestones/v4.8-ROADMAP.md`
- `.planning/milestones/v4.8-REQUIREMENTS.md`
- `.planning/milestones/v4.8-MILESTONE-AUDIT.md`
- `.planning/milestones/v4.8-phases/` (5 phase directories)
...
- Ported the Assembly measurement gizmo (Ctrl+Y, GLGizmoAssembly/ONLY_ASSEMBLY): GizmoAssemblyMeasure=19 enum, activability gating (AssembleView + explosion�?.0 + �? volumes), AssemblyMeasureGeometry C++ helper, overlay (dashed line + arrowheads + teal value box), and right-side 测量 panel (Phase 92).
