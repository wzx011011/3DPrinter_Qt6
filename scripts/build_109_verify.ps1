# Phase 109 CRITICAL-1 fix verify: build owzx_app_core + ViewModelSmokeTests + run ctest.
...
Write-Host "[109] Building owzx_app_core + ViewModelSmokeTests..." -ForegroundColor Cyan
& ninja -j16 owzx_app_core ViewModelSmokeTests QmlUiAuditTests PartPlateTests PrepareSceneDataTests 2>&1 | ForEach-Object { Write-Host $_ }
if ($LASTEXITCODE -ne 0) { Write-Host "[109] BUILD FAILED (exit $LASTEXITCODE)" -ForegroundColor Red; exit $LASTEXITCODE }
Write-Host "[109] Build OK" -ForegroundColor Green
